...
public class TFIDF {
	...
	private static double getInverseDocumentFrequency(String term, Map<String, DocumentData> documentResults) {
		...
		for (String document : documentResults.keySet()) {
			if (documentResults.get(document).getFrequency(term) > 0.0) {
				nt++;
			}
		}
	...
	private static Map<String, Double> getTerToInverseDocumentFrequencyMap(List<String> terms,
			Map<String, DocumentData> documentResults) {
		Map<String, Double> termToIDF = new HashMap<>();
		for (String term : terms) {
			termToIDF.put(term, getInverseDocumentFrequency(term, documentResults));
		}
		return termToIDF;
	}
	...
	public static Map<Double, List<String>> getDocumentsSortedByScore(List<String> terms,
		...
		Map<String, Double> termToInverseDocumentFrequency = getTerToInverseDocumentFrequencyMap(terms,
				documentResults);
		for (String document : documentResults.keySet()) {
